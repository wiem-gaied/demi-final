...
// Accent = gradient (background only)
...
function formatExtraInfo(extra_info) {
  try {
    const obj = typeof extra_info === "string" ? JSON.parse(extra_info) : extra_info;
...
function exportCSV(logs) {
  ...
  const rows = logs.map(l =>
    `"${l.id || ""}","${formatDate(l.created_at)}","${l.level || ""}","${l.source || ""}","${l.action || ""}","${(l.target || "").replace(/"/g, '""')}","${l.user_email || ""}","${l.role || ""}","${l.ip_address || ""}","${typeof l.extra_info === 'string' ? l.extra_info.replace(/"/g, '""') : JSON.stringify(l.extra_info || {}).replace(/"/g, '""')}"`
  );
...
function SrcTag({ source }) {
  return (
    <span style={{
      ...
      border: `0.5px solid ${C.border}`, fontFamily: "monospace"
    }}>
      {source || "system"}
    </span>
  );
    ...
    <div style={{
      ...
      <div style={{ padding: "12px 20px", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
        {[
          ...
          <div key={k}>
            <div style={{ fontSize: 10, fontWeight: 500, color: C.textMuted, marginBottom: 3 }}>{k}</div>
        ...
        {log.extra_info && (
          <div style={{ gridColumn: "1/-1", padding: "8px 12px", background: C.surface, borderRadius: 8 }}>
            <div style={{ fontSize: 10, fontWeight: 500, color: C.textMuted, marginBottom: 5 }}>ADDITIONAL DATA</div>
...
function LogRow({ log, expanded, onToggle }) {
  ...
  return (
    <React.Fragment>
      ...
      >
        <td style={{ padding: "10px 12px", fontSize: 11, color: C.textMuted, whiteSpace: "nowrap" }}>
        ...
        <td style={{ padding: "10px 12px" }}><ActionTag action={formatAction(log.action)} /></td>
        <td style={{ padding: "10px 12px", fontSize: 12, color: C.text, maxWidth: 280, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {log.target || "-"}
        </td>
        <td style={{ padding: "10px 12px", fontSize: 11, color: C.textMuted }}>{log.user_email}</td>
        <td style={{ padding: "10px 12px", fontSize: 10, color: C.textMuted }}>{log.role}</td>
        <td style={{ padding: "10px 12px", textAlign: "center", color: C.textMuted }}>{isExpanded ? "▲" : "▼"}</td>
      </tr>
      {isExpanded && (
        <tr>
...
function Pagination({ current, total, onPage }) {
  ...
  const btnStyle = (active, disabled = false) => ({
    ...
    border: `0.5px solid ${active ? C.wow : C.border}`,
    // Active button uses gradient background (allowed on bg), inactive stays white
    ...
    boxShadow: active ? `0 2px 8px ${C.accentGlow}` : "none",
  });
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 24px" }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        <button style={btnStyle(false, current === 1)} disabled={current === 1} onClick={() => onPage(current - 1)}>‹</button>
        {range.map((p, i) =>
          p === "…" ? (
            <span key={i} style={{ padding: "0 4px", fontSize: 12, color: C.textMuted }}>…</span>
          ) : (
            <button key={p} style={btnStyle(p === current)} onClick={() => onPage(p)}>{p}</button>
          )
        )}
        <button style={btnStyle(false, current === pages)} disabled={current === pages} onClick={() => onPage(current + 1)}>›</button>
      </div>
    </div>
  );
}
...
export default function Logs() {
  ...
  const fetchLogs = useCallback(async () => {
    try {
      ...
      else if (data?.data && Array.isArray(data.data)) logsData = data.data;
      else if (data?.logs && Array.isArray(data.logs)) logsData = data.logs;
  ...
  const uniqueRoles   = useMemo(() => Array.from(new Set(logs.map(l => l.role).filter(Boolean))).sort(), [logs]);

  const filtered = useMemo(() => {
    const now = Date.now();
    const cutoffs = { "1h": now - 3600000, "6h": now - 21600000, "24h": now - 86400000, "today": new Date().setHours(0,0,0,0) };
    let list = logs.filter(log => {
      ...
      if (search) {
        ...
        return (log.user_email||"").toLowerCase().includes(s) ||
    ...
    list.sort((a, b) => {
      ...
      if (sortField === "created_at") { va = va instanceof Date ? va.getTime() : 0; vb = vb instanceof Date ? vb.getTime() : 0; }
  ...
  // Button style: gradient bg when active (allowed), wow color text when inactive
  ...
  // th: use C.wow (solid) for active column — gradient not applicable to text
  const thStyle = (field) => ({
    padding: "10px 12px", textAlign: "left",
    fontSize: 11, fontWeight: 600,
    color: sortField === field ? C.wow : C.textMuted,
    background: C.surfaceAlt,
    borderBottom: `0.5px solid ${C.border}`,
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      {/* Header — title in solid C.wow, no gradient on text */}
      <div style={{
      ...
      <div style={{
        ...
        <div style={{ position: "relative", flex: 1, minWidth: 240 }}>
          <input
            ...
            style={{
              ...
              fontSize: 12, background: C.surfaceAlt, outline: "none",
            }}
            onFocus={e => { e.target.style.borderColor = C.wow; e.target.style.boxShadow = `0 0 0 3px ${C.accentGlow}`; }}
            onBlur={e => { e.target.style.borderColor = C.border; e.target.style.boxShadow = "none"; }}
          />
        </div>

        {/* Level pills — active uses solid bg color (not gradient), text uses matching color */}
        <div style={{
          display: "flex", gap: 4,
            ...
            return (
              <button
                ...
                style={{
                  ...
                  cursor: "pointer",
                  // Active: use bg from LEVEL_META or accentLight, text uses solid color
                  background: isActive ? (meta?.bg || C.accentLight) : "transparent",
                  color: isActive ? (meta?.color || C.wow) : C.textMuted,
                }}
              >
                {l === "all" ? "All" : l}
              </button>
        ...
        {/* Count chip — solid C.wow text on accentLight bg */}
        <span style={{
          fontSize: 12, color: C.wow, marginLeft: "auto",
          background: C.accentLight,
          padding: "4px 12px", borderRadius: 20, fontWeight: 600,
          border: `0.5px solid ${C.borderStrong || C.border}`,
        }}>
      ...
      {/* Table */}
      <div style={{
        margin: "16px 24px", background: C.surface,
        border: `0.5px solid ${C.border}`, borderRadius: 12, overflow: "auto",
        boxShadow: C.shadowMd,
      }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 800 }}>
          <colgroup>
            <col style={{ width: 170 }} /><col style={{ width: 85 }} />
            <col style={{ width: 85 }} /><col style={{ width: 85 }} />
            <col /><col style={{ width: 160 }} />
            <col style={{ width: 85 }} /><col style={{ width: 32 }} />
          </colgroup>
          <thead>
            <tr>
              {[
          ...
          <tbody>
            {pageSlice.length === 0 ? (
              ...
              pageSlice.map(log => (
                <LogRow key={log.id} log={log} expanded={expanded}
