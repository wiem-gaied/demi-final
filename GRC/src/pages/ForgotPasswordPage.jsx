...
function isPasswordStrong(pw, email) {
  if (pw.length < 16) return { valid: false, reason: "Minimum 16 characters required" };
  ...
  const commonWords = ["password", "admin", "user", "test", "welcome", "hello", "bonjour"];
  ...
  if (result.score < 3 && getAdvancedScore(pw) < 6) return { valid: false, reason: "Weak password" };
  return { valid: true };
}
...
const STRENGTH_CONFIG = [
  ...
  { min: 5, label: "Strong",      color: C.wow     },
  { min: 6, label: "Very strong", color: "#061585" },
];

function getStrengthConfig(score) {
  for (let i = STRENGTH_CONFIG.length - 1; i >= 0; i--) {
    if (score >= STRENGTH_CONFIG[i].min) return STRENGTH_CONFIG[i];
  }
...
const MODAL_CONTAINER = {
  ...
  boxShadow: `0 10px 40px rgba(59,111,255,0.10), 0 2px 8px rgba(0,0,0,0.06)`,
  position: "relative",
  display: "flex",
...
const LOGIN_BTN = {
  ...
  cursor: "pointer",
  fontFamily: "inherit",
  boxShadow: `0 4px 14px rgba(59,111,255,0.28)`,
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
      <div style={{
        ...
        boxShadow: `0 2px 8px rgba(59,111,255,0.15)`,
      }}>
        <ShieldCheck size={22} color={C.wow} />
      </div>
      <div>
        <div style={{ fontWeight: 800, fontSize: 19, color: C.text, fontFamily: "'DM Sans', sans-serif" }}>GRC Platform</div>
        <div style={{ fontSize: 12, color: C.textMuted }}>Governance · Risk · Compliance</div>
      </div>
    </div>
  );
}
...
function IconField({ icon: Icon, value, onChange, onKeyDown, type = "text", placeholder, rightSlot }) {
  ...
  return (
    <div style={{ position: "relative" }}>
      <Icon size={15} color={focused ? C.wow : C.textMuted}
        style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", transition: "color .15s", pointerEvents: "none" }} />
      <input
        style={{
          ...INPUT,
          borderColor: focused ? C.wow : C.border,
          boxShadow: focused ? `0 0 0 3px rgba(59,111,255,0.10)` : "none",
...
export default function ForgotPasswordPage() {
  ...
  const sendReset = async () => {
    ...
    try {
      ...
      if (!response.ok) { const data = await response.json(); throw new Error(data.message || "Error sending reset link"); }
      setScreen("sent");
    } catch (err) { setError(err.message || "Server error. Please try again."); }
  ...
  const resetPassword = async () => {
    ...
    if (!advancedCheck.valid) { setError(advancedCheck.reason || "Password does not meet security requirements"); return; }
    if (newPwd !== confirmPwd) { setError("Passwords do not match."); return; }
    setLoading(true);
    try {
      const response = await fetch("http://localhost:3000/api/reset/reset-password", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, password: newPwd }), credentials: "include",
      });
      if (!response.ok) { const data = await response.json(); throw new Error(data.message || "Error resetting password"); }
      setScreen("done");
    } catch (err) { setError(err.message || "Error updating password. Please try again."); }
    finally { setLoading(false); }
  };

  const Header = ({ title, sub }) => (
    <div style={{ padding: "28px 34px 20px", borderBottom: `1px solid ${C.border}` }}>
      <PlatformHeader />
      <div style={{ fontWeight: 700, fontSize: 16, color: C.text }}>{title}</div>
      <div style={{ fontSize: 13, color: C.textMid, marginTop: 2 }}>{sub}</div>
    </div>
  );

  const BackBtn = () => (
    <button onClick={() => navigate("/login")}
      style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 600, color: C.textMid, fontFamily: "inherit", padding: 0, marginTop: 16, transition: "color .15s" }}
      onMouseEnter={e => e.currentTarget.style.color = C.wow}
      ...
      <div style={MODAL_CONTAINER}>
        <div style={{ padding: "40px 34px", textAlign: "center" }}>
          <div style={{
            ...
            background: C.dangerLight, border: `2px solid #FECACA`,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <AlertCircle size={36} color={C.danger} />
          </div>
          <div style={{ fontWeight: 800, fontSize: 20, color: C.text, marginBottom: 8 }}>Invalid Reset Link</div>
          <p style={{ fontSize: 13, color: C.textMid, lineHeight: 1.65, margin: "0 0 28px" }}>
    ...
    return (
      <div style={MODAL_CONTAINER}>
        ...
        <div style={{ padding: "24px 34px 30px" }}>
          <p style={{ fontSize: 13, color: C.textMid, margin: "0 0 20px", lineHeight: 1.65 }}>
            Enter the email address associated with your account and we'll send you a reset link.
          </p>
          {error && !token && <div style={ERROR_BOX}><AlertCircle size={15} color={C.danger} /><span style={{ fontSize: 13 }}>{error}</span></div>}
          <label style={LBL}>Email address</label>
          <IconField icon={Mail} value={email} onChange={e => setEmail(e.target.value)}
            onKeyDown={e => e.key === "Enter" && sendReset()} type="email" placeholder="you@company.com" />
  ...
  if (screen === "reset") {
    return (
      <div style={MODAL_CONTAINER}>
        ...
        <div style={{ padding: "24px 34px 30px" }}>
          {error && <div style={ERROR_BOX}><AlertCircle size={15} color={C.danger} /><span style={{ fontSize: 13 }}>{error}</span></div>}

          {/* New password */}
          <div style={{ marginBottom: 14 }}>
            <label style={LBL}>New password</label>
            <div style={{ position: "relative" }}>
              <Lock size={15} style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: C.textMuted }} />
              <input
              ...
              <div style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", display: "flex", gap: "6px" }}>
                <button type="button"
                  onClick={() => { const p = generatePassword(); setNewPwd(p); setConfirmPwd(p); navigator.clipboard.writeText(p); }}
                  style={{
                    background: C.accentLight, border: `1px solid ${C.borderStrong}`,
                    borderRadius: "8px", padding: "4px 6px", cursor: "pointer",
                    fontSize: "10px", fontWeight: "600", color: C.wow,
                  }}>
                  Generate
                </button>
                ...
                <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                  {[0, 1, 2, 3, 4, 5].map((i) => (
                    <div key={i} style={{
                      ...
                      background: i < advancedScore ? strengthConfig.color : C.border,
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input
                ...
                style={{
                  ...INPUT,
                  borderColor: confirmPwd && newPwd !== confirmPwd ? C.danger
                    : confirmPwd && newPwd === confirmPwd ? C.success
                    : C.border,
                  paddingRight: 40,
                }}
              />
              <button type="button" onClick={() => setShowConfirm(!showConfirm)}
                style={{ position: "absolute", right: 11, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: C.textMuted, display: "flex" }}>
                {showConfirm ? <EyeOff size={15} /> : <Eye size={15} />}
              </button>
            </div>
            {confirmPwd && newPwd !== confirmPwd && (
              <div style={{ fontSize: 12, color: C.danger, marginTop: 5 }}>Passwords do not match</div>
            )}
            {confirmPwd && newPwd === confirmPwd && (
              <div style={{ fontSize: 12, color: "#061585", marginTop: 5, display: "flex", alignItems: "center", gap: 4 }}>
          ...
          {/* Requirements */}
          <div style={{ background: C.surfaceAlt, border: `1px solid ${C.border}`, borderRadius: 10, padding: "10px 14px", marginBottom: 18 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: C.textMuted, marginBottom: 8 }}>Security Requirements</div>
              ...
              {REQUIREMENTS.map((req) => {
                ...
                return (
                  <div key={req.id} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    ...
                    <span style={{ fontSize: 11, color: met ? C.wow : C.textMuted, fontWeight: met ? 500 : 400 }}>
                      {req.label}
                    </span>
                  </div>
                );
    ...
    return (
      <div style={MODAL_CONTAINER}>
        <div style={{ padding: "40px 34px 36px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
          <div style={{
            width: 72, height: 72, borderRadius: 20, marginBottom: 22,
            background: C.accentLight, border: `2px solid ${C.borderStrong}`,
            display: "flex", alignItems: "center", justifyContent: "center",
          }}>
            <Mail size={36} color={C.wow} />
          </div>
          <div style={{ fontWeight: 800, fontSize: 20, color: C.text, marginBottom: 8 }}>Check your email</div>
          <p style={{ fontSize: 13, color: C.textMid, lineHeight: 1.65, margin: "0 0 28px" }}>
            We've sent a password reset link to<br /><strong>{email}</strong>
          </p>
          <button onClick={sendReset}
            style={{ ...LOGIN_BTN, width: "auto", padding: "11px 32px", background: C.surfaceAlt, color: C.textMid, boxShadow: "none" }}>
            Send again
          </button>
          <div style={{ textAlign: "center" }}><BackBtn /></div>
        </div>
  ...
  return (
    <div style={MODAL_CONTAINER}>
      <div style={{ padding: "40px 34px 36px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
        <div style={{
          width: 72, height: 72, borderRadius: 20, marginBottom: 22,
          background: C.successLight, border: `2px solid #86EFAC`,
          display: "flex", alignItems: "center", justifyContent: "center",
        }}>
          <CheckCircle2 size={36} color={C.success} />
        </div>
        <div style={{ fontWeight: 800, fontSize: 20, color: C.text, marginBottom: 8 }}>Password updated!</div>
        <p style={{ fontSize: 13, color: C.textMid, lineHeight: 1.65, margin: "0 0 28px" }}>
          Your password has been successfully reset.<br />You can now sign in with your new credentials.
        </p>
        <button onClick={() => navigate("/login")}
