...
T.accent = `linear-gradient(135deg, ${T.wow}, ${T.warning})`;
// Couleur unie proche du dégradé (pour le texte / les bordures, où le dégradé est impossible)
T.accentSolid = "#2A4AD0";
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {[5, 30, 60, 120].map(t => (
                <button key={t} onClick={() => updateTimeout(t)} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: `2px solid ${sessionTimeout === t ? "transparent" : T.border}`, background: sessionTimeout === t ? T.accent : "transparent", color: sessionTimeout === t ? "#fff" : T.muted, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{t}m</button>
              ))}
            </div>
          </div>
        </div>
  ...
  return (
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 18, overflow: "hidden" }}>
      <div style={{ padding: "16px 24px", borderBottom: `1px solid ${T.border}`, display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ background: T.accent, color: "#fff", borderRadius: 10, padding: 8, display: "flex" }}>{icon}</div>
...
function Toggle({ value, onChange, color }) {
  return (
    <button onClick={() => onChange(!value)} style={{ width: 44, height: 24, borderRadius: 12, border: "none", background: value ? T.accent : "#CBD5E1", cursor: "pointer", position: "relative", transition: "background .2s", flexShrink: 0 }}>
          ...
          {tabs.map((t) => (
            <button
              ...
              style={{
                ...
                borderBottom:
                  tab === t.id
                    ? `3px solid ${T.accentSolid}`
                    : "3px solid transparent",
                background: "transparent",
                color: tab === t.id ? T.accentSolid : T.muted,
