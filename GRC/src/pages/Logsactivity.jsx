...
// Accent = dégradé (à n'utiliser QUE comme background)
C.accent = `linear-gradient(135deg, ${C.wow}, ${C.warning})`;
// Dégradé appliqué sur le TEXTE (via background-clip)
const ACCENT_TEXT = {
  background: C.accent,
...
const LEVEL_META = {
  ERROR:   { color: C.red,   bg: C.redLight,    border: "#FECACA", icon: "✕", label: "Error" },
  WARNING: { color: C.amber, bg: C.amberLight,  border: "#FCD34D", icon: "⚠", label: "Warning" },
...
function ActionTag({ action }) {
  const colors = {
    ...
    'LOGIN': { bg: C.accentLight, color: C.wow, dot: C.wow },
...
function LogRow({ log, expanded, onToggle }) {
  ...
  const levelColor = LEVEL_META[level]?.color || C.wow;
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <td style={{ padding: "11px 12px" }}><ActionTag action={formatAction(log.action)} /></td>
...
function Pagination({ current, total, onPage }) {
  ...
  const btnStyle = (active, disabled = false) => ({
    ...
    height: 34,
    borderRadius: 9,
    border: `1px solid ${active ? C.wow : C.border}`,
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? (meta?.color || C.wow) : C.textSub,
                  transition: "all 0.15s",
                  letterSpacing: "0.2px",
                }}
              >
              ...
              <tr>
                ...
                {[
                  ...
                  <th key={label} style={thStyle(field)} onClick={() => field && handleSort(field)}>
                    <span style={{ display: "flex", alignItems: "center", gap: 4, ...ACCENT_TEXT }}>
                      {label}
                      {field && sortField === field && (
                        <span style={{ ...ACCENT_TEXT, fontSize: 10 }}>{sortDir === "asc" ? "↑" : "↓"}</span>
                      )}
                    </span>
                  </th>
                ))}
